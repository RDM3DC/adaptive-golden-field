# tk_sweep_gamma_fixed.py
# Sweep Tk with fixed gamma, report KL_rate (per step), ΔAUC, lag; fit KL_rate ≈ E_max * Tk/(Tk+T0) + b.
...
# ---------- LIGO-like colored noise (for observables) ----------
...
def run_once_with_noise(Tk, gamma=0.10, T=100, N=101, ex=6.5, ey=4.0, rays=13, steps=340,
                        noise_sigma=0.05, seed=123, amp_phi=0.25, amp_focus=0.25):
    # simulate clean
    phi, foc, snaps = run_sim(T,N,ex,ey,rays,steps,noise_sigma,Tk,odd=gamma,seed=seed)
    # add LIGO-shaped noise to observables
    ...
    phiN = phi + n_phi*np.std(phi); focN = foc + n_foc*np.std(foc)
    ...
    # KL_rate (per step) on increments
    dphi = np.diff(phiN); dfoc = np.diff(focN)
    KL_rate = path_KL_gaussian(np.vstack([dphi, dfoc]).T) / max(len(dphi),1)
    # lag
    lag, xc = lag_by_xcorr(-phiN, focN, maxlag=8)
    return {
        "Tk": Tk, "KL_rate": KL_rate, "AUC_fwd": auc, "AUC_rev": aucr,
        "dAUC": auc-aucr, "lag": lag, "xcorr": xc
    }

# ---------- simple saturation fit: y ≈ Emax * Tk/(Tk+T0) + b (grid search) ----------
def fit_saturating(Tk, y):
    Tk = np.asarray(Tk, float); y = np.asarray(y, float)
    Emax_grid = np.linspace(0.0, max(y)*3 + 1e-9, 81)
    T0_grid   = np.linspace(0.1, 5.0, 81)
    b_grid    = np.linspace(min(y)-0.5*abs(min(y)), max(y)*0.25, 41)
    best = (1e99, 0,0,0)
    for E in Emax_grid:
        for T0 in T0_grid:
            frac = Tk/(Tk+T0)
            for b in b_grid:
                ...
                if err < best[0]:
                    best = (err, E, T0, b)
    err,E,T0,b = best
    SS_res = np.sum((y-(E*Tk/(Tk+T0)+b))**2)
    SS_tot = np.sum((y - y.mean())**2) + 1e-12
    R2 = 1.0 - SS_res/SS_tot
    return {"Emax":E, "T0":T0, "b":b, "R2":float(R2)}

# ---------- CLI ----------
def parse_args():
    ap = argparse.ArgumentParser()
    ap.add_argument("--outdir", type=str, default="outputs/tk_sweep")
    ap.add_argument("--Tk_list", type=str, default="0.5,0.7,1.0,1.3,1.7,2.2,3.0,4.0,5.0")
    ...
    ap.add_argument("--amp_phi", type=float, default=0.25)
    ap.add_argument("--amp_focus", type=float, default=0.25)
    return ap.parse_args()

def main():
    args = parse_args()
    os.makedirs(args.outdir, exist_ok=True)
    Tk_vals = [float(s) for s in args.Tk_list.split(",") if s.strip()!=""]
    rows = []
    for Tk in Tk_vals:
        m = run_once_with_noise(Tk, gamma=args.gamma, T=args.T, N=args.grid, rays=args.rays, steps=args.steps,
                                noise_sigma=args.noise, seed=args.seed,
                                amp_phi=args.amp_phi, amp_focus=args.amp_focus)
        rows.append(m)
        print(f"Tk={Tk:.2f} -> KL_rate={m['KL_rate']:.3f}, dAUC={m['dAUC']:.3f}, lag={m['lag']:.2f}")

    # CSV
    keys = ["Tk","KL_rate","AUC_fwd","AUC_rev","dAUC","lag","xcorr"]
    csv_path = os.path.join(args.outdir, "tk_sweep_gamma_fixed.csv")
    with open(csv_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=keys); w.writeheader(); w.writerows(rows)

    # Fit + plots
    Tk_arr = np.array([r["Tk"] for r in rows], float)
    KL_arr = np.array([r["KL_rate"] for r in rows], float)
    fit = fit_saturating(Tk_arr, KL_arr)
    print(f"Fit: Emax={fit['Emax']:.3f}, T0={fit['T0']:.3f}, b={fit['b']:.3f}, R2={fit['R2']:.3f}")

    # KL_rate vs Tk + fit
    Tk_dense = np.linspace(min(Tk_arr), max(Tk_arr), 300)
    KL_fit   = fit["Emax"]*Tk_dense/(Tk_dense + fit["T0"]) + fit["b"]
    plt.figure(figsize=(6.0,4.2))
    plt.plot(Tk_arr, KL_arr, marker='o', linestyle='none', label="KL_rate")
    plt.plot(Tk_dense, KL_fit, label=f"fit Emax={fit['Emax']:.2f}, T0={fit['T0']:.2f}, R2={fit['R2']:.2f}")
    plt.xlabel("Tk"); plt.ylabel("KL rate (per step)")
    plt.title("Entropy production vs Tk (γ fixed)")
    plt.legend(); plt.tight_layout()
    plt.savefig(os.path.join(args.outdir, "Tk_vs_KLrate_fit.png"), dpi=200); plt.close()

    # ΔAUC vs Tk
    dAUC = np.array([r["dAUC"] for r in rows], float)
    plt.figure(figsize=(6.0,4.2))
    plt.plot(Tk_arr, dAUC, marker='o', linestyle='none')
    plt.xlabel("Tk"); plt.ylabel("ΔAUC (fwd - rev)")
    plt.title("ROC gap vs Tk")
    plt.tight_layout()
    plt.savefig(os.path.join(args.outdir, "Tk_vs_dAUC.png"), dpi=200); plt.close()

    # lag vs Tk
    lag = np.array([r["lag"] for r in rows], float)
    plt.figure(figsize=(6.0,4.2))
    plt.plot(Tk_arr, lag, marker='o', linestyle='none')
    plt.xlabel("Tk"); plt.ylabel("lag (frames)")
    plt.title("ϕ_a → focus lag vs Tk")
    plt.tight_layout()
    plt.savefig(os.path.join(args.outdir, "Tk_vs_lag.png"), dpi=200); plt.close()
